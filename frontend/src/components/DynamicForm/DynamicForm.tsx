...
function ObjectArray({field,name}:{field:SchemaField;name:string}){const {control}=useFormContext();const {fields,append,remove}=useFieldArray({control,name});const children=childFields(field);return <div className="object-array"><Space direction="vertical" style={{width:'100%'}}>{fields.map((item,index)=><Card key={item.id} size="small" title={`${field.label} ${index+1}`} extra={<Button danger type="text" onClick={()=>remove(index)}>删除</Button>}><FieldList fields={children} prefix={`${name}.${index}`} /></Card>)}</Space><Button style={{marginTop:10}} onClick={()=>append({})}>添加{field.label}</Button></div>}
...
function FieldControl({field,name}:{field:SchemaField;name:string}){const {control}=useFormContext();if(field.type==='object')return <Card size="small"><FieldList fields={childFields(field)} prefix={name}/></Card>;if(field.type==='object_array')return <ObjectArray field={field} name={name}/>;return <Controller name={name} control={control} render={({field:input})=>{
  ...
  if(field.type==='number')return <InputNumber {...input} value={input.value as number|null} min={field.min} max={field.max} disabled={field.readonly} style={{width:'100%'}}/>
  ...
  return <Input {...input} value={(input.value as string)??''} disabled={field.readonly}/>
}}/>}
