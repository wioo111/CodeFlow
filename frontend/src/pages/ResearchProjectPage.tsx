...
export function ResearchProjectPage(){
  ...
  const [versions,setVersions]=useState<DatasetVersion[]>([]);const [items,setItems]=useState<ResearchSample[]>([]);const [total,setTotal]=useState(0);const [loading,setLoading]=useState(true)
  ...
  useEffect(()=>{if(!dataset)return;api.researchSamples(Number(projectId),{dataset_version_id:String(dataset)}).then((queue)=>{setItems(queue.items);setTotal(queue.total)}).finally(()=>setLoading(false))},[dataset,projectId])
  ...
  return <main className="page research-overview">
    ...
    <Row gutter={16} style={{marginBottom:18}}><Col span={6}><Card><Statistic title="当前队列" value={total} suffix="条"/></Card></Col><Col span={6}><Card><Statistic title="待处理" value={items.filter(item=>item.status==='pending').length}/></Card></Col><Col span={6}><Card><Statistic title="进行中" value={items.filter(item=>item.status==='in_progress').length}/></Card></Col><Col span={6}><Card><Statistic title="已提交" value={items.filter(item=>item.status==='submitted').length}/></Card></Col></Row>
